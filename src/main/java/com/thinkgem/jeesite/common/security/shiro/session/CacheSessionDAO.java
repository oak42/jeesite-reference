...
public class CacheSessionDAO extends EnterpriseCacheSessionDAO implements SessionDAO {
	 ...
	 * 获取活动会话
	 * @param includeLeave 是否包括离线（最后访问时间大于3分钟为离线会话）
	 * @param principal 根据登录者对象获取活动会话，若null即表明此约束条件缺失，即不起约束作用，则返回系统中所有active session
	 * @param filterSession 不为空，则过滤掉（不包含）这个会话。
	 * @return
	 */
    /*
      里面getActiveSessions()，即CachingSessionDao.getActiveSessions() api doc：
      ...
      Subclass implementations may wish to override this method to retrieve them in a different way, perhaps by an RDBMS query or by other means.
    */
	@Override
	public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession) {
		...
		for (Session session : getActiveSessions()){
			...
			if (includeLeave || DateUtils.pastMinutes(session.getLastAccessTime()) <= 3){	//【扣】hardcoded进来了啊，明明可以引用配置好的jeesite.properties中session.sessionTimeout=1800000？
